...
class KYRobertaM2Transfer(nn.Module):
    def __init__(self, ckpt_path, class_num, drop_rate=0.1, freeze=False):
        ...
        self.fuse_linear = nn.Linear((acoustic_config.hidden_size+semantic_config.hidden_size)*2,
                                     (acoustic_config.hidden_size+semantic_config.hidden_size)*2)

        self.classifier = nn.Linear((acoustic_config.hidden_size+semantic_config.hidden_size)*2, class_num, bias=False)
    ...
    def forward(self, 
        ...
        # we first try with the concat
        fuse_encode = torch.cat([text_att, acoustic_pool, text_max, acoustic_max],dim=-1)
        fuse_encode = self.fuse_linear(fuse_encode)
        fuse_encode_act = torch.tanh(fuse_encode)
